# -*- coding: utf-8 -*-
"""
Created on Sun Nov 12 22:23:48 2023
...
class EvaluationMetrics:
    ...
    def nmse(self, actual, pred):
        actual, pred = np.array(actual), np.array(pred)
        return np.mean(actual - pred)**2 / np.mean(actual, axis=0)
    def nmse1(self, actual, pred):
        actual, pred = np.array(actual), np.array(pred)
        return np.sum(actual - pred) /np.sum(actual)
    ...
    def calculate_multi_step_all_samples_rmse(self,predictions, targets):
        ...
        for i in range(num_steps):
            ...
            mape_step=self. nmse (target_step,prediction_step)
...
def write_metrics_to_csv(filename, rmse, nrmse, mape, smape, r2, extra_data):
    ...
    with open(filename, mode=mode, newline='') as file:
        ...
        if mode == 'w':
            writer.writerow(["RMSE", "nRMSE", "FS_nrmse", "sMAPE", "R2"])  # 写入标题行
        ...
        writer.writerow(["", "", "", "", "", ""]) 
        writer.writerow(["all_samples_rmse", "nrmse_mean", "FS_nrmse_mean", "sampe_mean", "R2_mean"]) 
...
print('FS_nrmse_mean: %.3f' % all_samples_mape_mean)
