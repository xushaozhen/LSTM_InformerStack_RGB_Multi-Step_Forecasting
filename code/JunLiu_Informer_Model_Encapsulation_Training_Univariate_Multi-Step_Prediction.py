...
def _process_one_batch(dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
       ...
       if padding==0:
           ...
           dec_inp = torch.ones([batch_y.shape[0], out_len, batch_y.shape[-1]]).float()
       # dec_inp = torch.cat([batch_y[:,:self.args.label_len,:], dec_inp], dim=1).float().to(self.device)
       dec_inp = torch.cat([batch_y[:,:12,:], dec_inp], dim=1).float().to(device)
       # encoder - decoder
       if use_amp:
...
def format_time(seconds):
    ...
    return time
def _get_data(root_path="./",data_path="flo2_nowindspeed.csv",
              flag="test",size=[seq_len,label_len,out_len],
